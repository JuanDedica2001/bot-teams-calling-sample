                    ...
                    switch (action)
                    {
                        case "create":
                            ...
                            if (call != null)
                            {
                                ...
                                var media = new MediaSession(_graphLogger, new Guid(callId), audioSettings);
                                IAudioSocket audioSocket = media.AudioSocket;
                                if (audioSocket == null)
                                {
                                    await turnContext.SendActivityAsync(MessageFactory.Attachment(adaptiveCardFactory.CreateTestCard()));
                                }
                                return await CreateTaskModuleMessageResponse("Working on that, you can close this dialog now: " + call.Id);
                            }
                            break;
                        case "transfer":
